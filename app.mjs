...
function pickStrain1({ strain }) /* : Node */ {
  switch (strain) {
    case "Sativa":
      return builder.makeNode(["Strain"], { NAME: "Sativa" });
    case "Indica":
...
function pickCultivar1({ cultivar }) /* : Node */ {
  const node = builder.makeNode(["Cultivar"], { NAME: cultivar });
  return node;
}
...
function pickManufacturer1({ product }) /* : Node */ {
  // parse 'Khiron THC Rich'
...
function pickDispensary1({ dispensary }) {
  return builder.makeNode(["Dispensary"], { NAME: dispensary });
}
  ...
  const newEnode = builder.makeEnhancedNode(
    ...
    [
      ...
      builder.makeRelationshipCandidate(
        ...
        {},
        "outbound",
        pickDispensary(product.dispensary)
      ),
    ]
  );
  // log(newEnode)
...
function productToEnode1(product) {
  // log(endNode)
  const newEnode = builder.makeEnhancedNode(
    builder.makeNode(
      ["Product"],
